...
def get_config():
    configfile = os.path.abspath("chat.config")

    if not os.path.isfile(configfile):
        st.error(f"Configuration file not found: {configfile}")
        st.stop()

    last_modified = os.path.getmtime(configfile)

    if "config_last_modified" not in st.session_state:
        st.session_state.config_last_modified = 0

    if last_modified != st.session_state.config_last_modified:
        st.session_state.configuration = dict(toml.load(configfile))
        st.session_state.config_last_modified = last_modified

    return st.session_state.configuration
...
feature_flags = configuration.get("feature_flags", {})
ui_config = configuration.get("ui", {})


STORE_DIR = config.get("store_dir", "store")
CHAT_FILE = os.path.join(STORE_DIR, config.get("chat_file", "chats.pkl"))
...
@st.cache_resource
def load_dataset():
    csv_path = os.path.join(STORE_DIR, config.get("merged_data_file", "merged_data.csv"))
...
with col2:
    ...
    st.caption("(LLM-Powered(GPT-OSS:20B) Pandas Execution Engine)")
...
with st.sidebar:
    ...
    for chat_key, messages in st.session_state.chats.items():
        ...
        with col2:
            if feature_flags.get("show_delete_chat", False):
    ...
    for i, chat in enumerate(st.session_state.chats[st.session_state.active_chat]):
        ...
        with col2:
            with st.chat_message("user"):
                ...
                if feature_flags.get("enable_edit", True):
...
if feature_flags.get("enable_edit", True) and st.session_state.edit_index is not None:
    st.divider()
...
if "question_to_run" in st.session_state:
    ...
    def process_question():
        ...
        sample = df.head(ui_config.get("max_preview_rows", 3)).to_markdown(index=False)
